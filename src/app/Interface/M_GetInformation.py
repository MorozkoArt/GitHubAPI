...
def print_assessment(user, assessment):
    ...
    x.add_row(["Account creation date\nLast update date\nAccount age", f"{user.created_at}\n{user.updated_at}\n{user.month_usege} Month(s)", round(assessment_profile_dict.get("month_usege"), 2)])
    if len(user.repos_user) != 0:
        ...
        x.add_row(["Average number of working days over the repository", round(user.avg_a_days, 2), round(assessment_profile_dict.get("avg_a_days"), 2)])
        x.add_row(["Average number of contributors in each repository", round(user.avg_cont, 2), round(assessment_profile_dict.get("avg_cont"), 2)])
        x.add_row(["Average number of views in each repository", round(user.avg_views, 2), round(assessment_profile_dict.get("avg_views"), 2)])
        x.add_row(["Number of stars", user.stars, round(assessment_profile_dict.get("stars"), 2)])
        x.add_row(["Number of forks", user.forks, round(assessment_profile_dict.get("forks"), 2)])
